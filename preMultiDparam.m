...
function [sw_md,sw_nmd,mweightvec,halfedgepos,multidsign,amthe_well] = ...
...
%Initialize "sw_left" and "sw_right". They store, respectively,
%the saturation in left and right sides for all half-edges.
sw_md = zeros(length(nsurn1),1);
sw_nmd = sw_md;
%Initialize "multidsign". If the flow came from counter clockwise way in 
%the interaction region, it receives "1" (conventional). It receives "0" 
%when the flow cames from the clockwise way.
multidsign = ones(length(nsurn1),1);

%The same occurs for "mweightvec" and "halfedgepos"
mweightvec = sw_md; 
halfedgepos = sw_md;

%Initialize the auxiliary counters "l"
l = 1;
amthe_well = 0;
    ...
    for i = 1:lengnsurn
        ...
        if localrowposit(i) <= bedgesize
            ...
            if (wellprodkey == 1 && all(ismember(vertices,selectednode))) ...
                ...
                knownsat = satonedges(localrowposit(i));

                %auxiliary "verticescoord" (extrapol. over the half-edge)
                auxvertcoord = [coord(inode,:); mean(verticescoord,1)];
            
                %Get the saturation in the:  
                boolmlp = length(mlplimiter) == 1;
                mlpbyelem = mlplimiter(boolmlp + (1 - boolmlp)*adjelem,:);
                %Store the saturation calculated in the current state 
                %(left).
                ...
                unknownsat = swonadjedge;  
            
                %Store the saturation calculated in the current state 
                %(left).
                sw_md(l) = ...
                    knownsat*booleankey + unknownsat*(1 - booleankey);
        ...
        %The half-edge belongs to "inedge"
            ...
            if (wellprodkey == 1 && all(ismember(vertices,selectednode))) ...
                ...
                if rownumber <= bedgesize
                    ...
                    booleankey = ...
                        (inode == inedge(rownumber - bedgesize,1));
                ...
                boolmlp = length(mlplimiter) == 1;
                mlpbyelem = ...
                    mlplimiter(boolmlp + (1 - boolmlp)*elemeval(1),:);
                %Store the saturation calculated in the current state 
                %(left).
                sw_md(l) = getsatonedge(elemeval,vertices,auxvertcoord,...
                    taylorterms,Sw,limiterflag,order,constraint,...
                    flagknownvert,satonvertices,mlpbyelem,...
                ...
                %(right).
                sw_nmd(l) = getsatonedge(fliplr(elemeval),vertices,...
                ...
                %Define the way for "multidsign". (1) - counter-clockwise;
                %(0) clockwise;
                multidsign(l) = booleanupst; 
        ...
        %Update the auxiliary counters "l" and "u"
        l = l + 1*booleaninc;
        u = u + 1*(getvalkey == 1);
