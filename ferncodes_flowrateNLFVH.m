function [flowrate,flowresult]=ferncodes_flowrateNLFVH(p, pinterp, parameter,mobility)
global inedge coord bedge bcflag phasekey centelem gravresult gravrate
...
for ifacont=1:bedgesize
    ...
    if bedge(ifacont,5)>200
        ...
        flowrate(ifacont,1)=mobonface*normcont*(parameter(1,1,ifacont)*(p(lef)-pinterp(parameter(1,3,ifacont)))+...
                                                    parameter(1,2,ifacont)*(p(lef)-pinterp(parameter(1,4,ifacont))))+gravresult(ifacont,1);        
    end
...
for iface=1:inedgesize
    ...
    norma=norm(vd1);
    ifactual=iface+bedgesize;%size(bedge,1);
   ...
   flowrate(iface+size(bedge,1),1)=mobonface*(ALL*p(lef)-ALR*p(rel))+gravrate(ifactual,1); 
