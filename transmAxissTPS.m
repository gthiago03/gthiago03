...
function [transmvecleft,transmvecright,knownvecleft,knownvecright,...
    ...
    bodyterm] = transmAxissTPS(kmap,benchkey,phasekey,g,q,r0)
%Define global parameters:
global coord elem nsurn1;
...
for inode = 1:size(coord,1)
    ...
    [subunknAleft,subunknAright,subauxBleft,subauxBright,subunknC,...
        ...
        calclittlematrix(inode,kmap,benchkey,phasekey,esurn,nsurn,Fg,...
        g,q,r0);
...
function[gradP] = calcgradP(r0,elemeval,inode,zetanode,netanode)
%Define global variables:
global coord centelem;

%Get the area of the triangle which define the linear function.
%Define the vectors "vec1" and "vec2":
%Get the coordinate of each half-edge
midcoord1 = midedge(inode,zetanode);
midcoord2 = midedge(inode,netanode);
%Get the centroid coordinate
centcoord = centelem(elemeval,:);
%Define the vectors:
vec1 = midcoord1 - centcoord;
vec2 = midcoord2 - centcoord;
%Calculate the cross product
crossprod = cross(vec1,vec2);
%Calculate the area
Ac = 0.5*(abs(crossprod(3)));

%Get the ray of the linear function centroid
rc = r0 + mean([centcoord(1) midcoord1(1) midcoord2(1)]);

%Initialize "b". It allow the coordinate "z" does not suffer the influence
%of the ray
b = [1; 0];

%Calculate the normals to each face of the triangular function
%Define a rotation matrix "R"
R = zeros(3);
R(1,2) = 1;
R(2,1) = -1;
%Rotate "vec1" and "vec2"
vec1 = R*vec1;
vec2 = R*vec2;
%Verify if the normal calculated to be turned to correct direction. 
%For this, we use the inner product of "pointcorrdir" with normal calcul.
%Define "pointcorrdir"
pointcorrdir = centcoord - coord(inode,:); 
%Correct the vector orientation if necessary
vec1 = vec1(1:2)*sign(dot(pointcorrdir,vec1));
vec2 = vec2(1:2)*sign(dot(pointcorrdir,vec2));
%Calculate the third vector ("vec3") by using the Euler identity:
vec3 = -(vec1 + vec2);

%Calculate the midedge for each triangular edge
midvec1 = 0.5*(centcoord + midcoord1); 
midvec2 = 0.5*(centcoord + midcoord2); 
midvec3 = 0.5*(midcoord1 + midcoord2); 
%Get the ray of each edge which compose the triangle:
rvec1 = r0 + midvec1(1);
rvec2 = r0 + midvec2(1);
rvec3 = r0 + midvec3(1);

%Define an aditional term associated to "b"
bterm = (2*Ac/3)*b;
%Finally calculate the gradient:
gradP = (1/(2*rc*Ac))*[(rvec1*vec1' + rvec2*vec2' - bterm) ...
    (rvec1*vec1' + rvec3*vec3' - bterm) ...
    (rvec3*vec3' + rvec2*vec2' - bterm)];
...
function [subunknAleft,subunknAright,subauxBleft,subauxBright,subunknC,...
    ...
    g,q,r0)
%Define global parameters:
global elem bedge inedge normals bcflag; 
...
for insurn = 1:length(nsurn)
    ...
    %Get the midedge coordinate. It is used in radial coordinate version:
    %It is the ray from "0,0" untill the middle of edge evaluated 
    %(the same value is attributed to both half-edges belonging to edge
    %IJ). It is add to "r0"
    midedgcoord = midedge(inode,inodesurn);

    %Verify if the half-edge evaluated belongs to "bedge". Thus, we
    %must find out the "bedge"'s row
    bedgerow = all(ismember(bedge(:,1:2),[inode inodesurn]),2);
    ...
    if any(bedgerow)
        ...
        %"gradP" obtain the gradient of pressure (for the left element) 
        gradPleft = calcgradP(r0,leftelemeval,inode,zetanode,netanode(1));
        ...
        if norm(g) ~= 0
            ...
            edgemap(insurn) = find(bedgerow == 1);
        ...
        %Attribute, to the flow on the half-edge, the parameter "rface".
        %Define the ray untill the face "rface" 
        rface = r0 + midedgcoord(1);

        %------------------------------------------------------------------
        %Fill both the matrix "subunknAleft" and the matrix "subauxBleft"
            
        %Calculate "k.nablaP" to left element
        knablap = rface*(kleft*gradPleft);
    ...
    %The HALF-EDGE BELONGS to INERNAL edges        
        ...
        inedgerow = find(all(ismember(inedge(:,1:2),[inode inodesurn]),2));
        ...
        %"gradP" obtain the gradient of pressure (for the left element) 
        gradPleft = calcgradP(r0,leftelemeval,inode,zetanode,netanode(1));
        %"gradP" obtain the gradient of pressure (for the right element) 
        gradPright = ...
            calcgradP(r0,rightelemeval,inode,zetanode,netanode(2));
        ...
        knablap = rface*(kright*gradPright);
