...
function [coord,centelem,centface,elem,esurn1,esurn2,nsurn1,nsurn2,bedge,inedge,...
    normals,un_normals, esureface1,esureface2,esurefull1,esurefull2,elemarea,dens,visc,...
...
[head,benchkey,pathstore,numcase,complemfile,phasekey,xyrz,r0,symaxe,...
    ...
    bcflag,numbcnode,bcnodepointer,courant,totaltime,numwell,well, nltol, maxiter,acel] = ...
    getdatafile;
...
if strcmp(keyopen,'y') && strcmp(keyfile,'n')
    %Build the command
    command = ['gmsh.exe "' mainpath '\' char(mshfile') '"'];
...
elseif strcmp(keyopen,'y') && strcmp(keyfile,'y')
    %Build the command
    command = ['gmsh.exe "' verifymshfile '"'];
    %Open gmsh throughout system prompt
    system(command);
...
%--------------------------------------------------------------------------
%"centface" matrix - It get the centroid coordinate for each face 

centface = getcentface(coord,bedge,inedge);

%Gives the information of "centface" generated
disp('"centface" was generated!');
...
if strcmp(xyrz,'xy')
    ...
    un_normals = calcunnormals(coord,centelem,bedge,inedge);
%When we have a 'rz' coordinate system
else
...
disp('"unit normals" was generated!');
...
function [head,benchkey,pathstore,numcase,complemfile,phasekey,xyrz,r0,...
    ...
    well,nltol,maxiter,acel] = getdatafile
...
function [un_normals] = calcunnormals(coord,centelem,bedge,inedge)
%Define the matrix rotation
R = zeros(3);
R(1,2) = 1;
R(2,1) = -1;
...
%Initialize "normals"
un_normals = zeros(size(bedge,1) + size(inedge,1),3);
...
for iflowb = 1:size(bedge,1)
    ...
    %Correct (if necessary) the normal orientation
    un_normals(iflowb,:) = ...
        un_normals(iflowb,:)*sign(dot(pointer,un_normals(iflowb,:)));
    
    un_normals(iflowb,:) = un_normals(iflowb,:) / ...
        norm((coord(bedge(iflowb,2),:) - coord(bedge(iflowb,1),:))');
...
for iflowin = 1:size(inedge,1)
    %Fill "normals"
    un_normals(size(bedge,1) + iflowin,:) = ...
        R*(coord(inedge(iflowin,2),:) - coord(inedge(iflowin,1),:))';
    
    un_normals(size(bedge,1) + iflowin,:) = un_normals(size(bedge,1) + iflowin,:) /...
        norm((coord(inedge(iflowin,2),:) - coord(inedge(iflowin,1),:))');
...
%--------------------------------------------------------------------------
%FUNCTION "getcentface"
%---------------'-----------------------------------------------------------

%This function calculate the coordinate of center's edge evaluated 
%(xunkn, yunkn, zunkn). A vector "coordunkn" (3x1) is returned.
%"coordunkn(1)" does mean the coordinate x of unknown point.
%The inflow data are "bedge" and "inedge" which are vectors with all nodes which
%constitute the edge evaluated.

function [centface] = getcentface(coord,bedge,inedge)                         
    %Initialize the matrix "centface"

    centface = zeros(size(bedge,1)+size(inedge,1),3);

    %Define the coordinate to each edge
    for ibedge = 1:(size(bedge,1))

        p1 = coord(bedge(ibedge,1),:);
        p2 = coord(bedge(ibedge,2),:);

        centface(ibedge,:)=p2+(p1-p2)*0.5;

    end  %End of FOR (each element)
    
    for iinedge = 1:(size(inedge,1))

        p1 = coord(inedge(iinedge,1),:);
        p2 = coord(inedge(iinedge,2),:);

        centface(iinedge+size(bedge,1),:)=p2+(p1-p2)*0.5;
