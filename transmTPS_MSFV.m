...
function [transmvecleft,transmvecright,knownvecleft,knownvecright,...
    ...
    bodyterm] = transmTPS_MSFV(kmap,q,knownboundlength,coordrowson)
%Define global parameters:
global elem bedge inedge nsurn1 phasekey keygravity;

%MULTISCALE APPLICATION:
auxcoordsize = length(coordrowson);
...
%Initialize "maptransm". 
%It stores the pointer to "transmvec" (MSFV - MODIFIED)
maptransm = zeros(auxcoordsize + 1,1);
mapinv = maptransm;
...
for i = 1:auxcoordsize  %MSFV MODIFIED
    %MSFV MODIFIED. Attribute the "coord" row "turned on" to "inode"
    inode = coordrowson(i);
    ...
    [subunknAleft,subunknAright,subauxBleft,subauxBright,subunknC,...
        ...
        calclittlematrix(inode,ibedg,iinedg,kmap,phasekey,esurn,nsurn,...
        Fg,q);
...
function [subunknAleft,subunknAright,subauxBleft,subauxBright,subunknC,...
    subauxD,getinfoedge,Fg,bodytermleft,bodytermright,edgemap] = ...
    calclittlematrix(inode,ibedg,iinedg,kmap,phasekey,esurn,nsurn,Fg,q)
%Define global parameters:
global elem bedge inedge normals bcflag g; 
...
for insurn = 1:length(nsurn)
    ...
    %The HALF-EDGE BELONGS to INERNAL edges        
        ...
        inedgerow = ...
            iinedg(all(ismember(inedge(:,1:2),[inode inodesurn]),2));
