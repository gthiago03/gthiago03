...
global coord elem esurn1 esurn2 nsurn1 nsurn2 
...
P=zeros(nsurn2(No+1)-nsurn2(No),3); % vetor de pontos na vizinhança do nó "ni".
T=zeros(nsurn2(No+1)-nsurn2(No),3); % vetor de pontos dinamicos na vizinhança do nó "ni".
...
for i=1:size(O,1)
    ...
    if elem(esurn1(esurn2(No)+i),4)==0 % lenbrando que o quarta columna
        b=3;                  
    else
    ...
    for j=1:b
        O(i,1)=O(i,1)+(coord(elem(esurn1(esurn2(No)+i),j),1)/b);
        O(i,2)=O(i,2)+(coord(elem(esurn1(esurn2(No)+i),j),2)/b);
        O(i,3)=O(i,3)+(coord(elem(esurn1(esurn2(No)+i),j),3)/b);
    end
end
